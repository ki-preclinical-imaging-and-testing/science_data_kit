"""
Local Storage Plugin for Science Data Kit.

This module provides a plugin for accessing files on the local filesystem.
...
@register_plugin
class LocalStoragePlugin(FilesystemPluginInterface):
    """
    Plugin for accessing files on the local filesystem.
    
    This plugin provides access to files and directories on the local filesystem,
    with security checks to prevent access to unauthorized paths.
    """
    
    def __init__(self):
        """Initialize the plugin."""
        self._name = "local_storage"
        self._version = "1.0.0"
        self._description = "Plugin for accessing files on the local filesystem"
        ...
        return PluginConfigSchema(
            fields=[
                ...
                ConfigField(
                    ...
                    array_item_type=ConfigFieldType.DIRECTORY_PATH,
                    description="Additional allowed paths (for security)",
                    required=False,
                    default=[],
                ),
                ConfigField(
    ...
    def list_directory(self, path: str) -> List[Dict[str, Any]]:
        ...
        if not self._is_path_allowed(str(full_path)):
            raise PermissionError(f"Access to path not allowed: {full_path}")
    ...
    def read_file(self, path: str) -> bytes:
        ...
        # Check if path is allowed
        if not self._is_path_allowed(str(full_path)):
            raise PermissionError(f"Access to file not allowed: {full_path}")
    ...
    def create_directory(self, path: str) -> bool:
        ...
        # Check if path is allowed
        if not self._is_path_allowed(str(full_path)):
            raise PermissionError(f"Access to directory not allowed: {full_path}")
    ...
    def _is_path_allowed(self, path: str) -> bool:
        ...
        if os.path.commonpath([path, str(self._root_dir)]) == str(self._root_dir):
            return True
