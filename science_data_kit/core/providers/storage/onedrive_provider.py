"""
OneDrive Storage Provider for Science Data Kit

This module provides an OneDrive storage provider for accessing files
stored in OneDrive through the Microsoft Graph API.
"""
...
class OneDriveProvider(BaseProvider):
    """
    OneDrive storage provider for file access through Microsoft Graph API.

    This provider allows access to OneDrive files and folders
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: Configuration dictionary containing:
                   ...
                   drive_id: OneDrive drive ID (optional)
        """
    ...
    async def initialize(self) -> bool:
        """
        Initialize OneDrive storage access.
        ...
        try:
            ...
            # If drive_id is not provided, use the user's default drive
            if not self.drive_id:
                # Get the user's default drive
                me = self.manager.get_me()
                if not me or "id" not in me:
                    print("Failed to get user information")
                    return False

                # Get the user's drives
                drives = self.manager.execute_query("/me/drives")
                if not drives or "value" not in drives or not drives["value"]:
                    print("No OneDrive drives found")
                    return False

                # Use the first drive (usually the default OneDrive)
            ...
            print(f"Error initializing OneDrive provider: {str(e)}")
            return False

    async def health_check(self) -> bool:
        """
        Verify OneDrive storage access is working.
        ...
        try:
            ...
            print(f"OneDrive health check failed: {str(e)}")
            return False
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_size": "100GB"  # OneDrive file size limit
        }
    ...
    async def list_files(self, folder_path: str = "", file_types: List[str] = None) -> List[Dict[str, Any]]:
        ...
        Args:
            folder_path: Path to the folder in OneDrive (item ID or path)
        ...
        """
        if not self.is_initialized or not self.manager:
            raise Exception("OneDrive provider not initialized")
    ...
    async def download_file_data(self, file_path: str) -> pd.DataFrame:
        ...
        Args:
            file_path: Path to the file in OneDrive (item ID)
