    ...
    def handle_request(self, method: str, path: str, params: Dict[str, Any], 
        ...
        try:
            ...
            if method == "GET":
                ...
                return self.get_session(token, params)
            elif method == "POST":
                ...
                if path.endswith("/login"):
                    ...
                    return self.create_session(token, body or {})
            elif method == "PUT":
                # Update session
                token = self.auth.authenticate_and_authorize(headers, APIPermission.WRITE)
                return self.update_session(token, body or {})
            elif method == "DELETE":
                # Delete session
                if path.endswith("/logout"):
                    ...
                    return self.delete_session(token, params)
            else:
                raise APIError(
        ...
        try:
            ...
            if method == "GET":
                ...
                if path.endswith("/query"):
                    return self.execute_query(token, params)
                else:
                    return self.get_database_info(token, params)
            elif method == "POST":
                # Execute a query or create a database
                token = self.auth.authenticate_and_authorize(headers, APIPermission.WRITE)
                if path.endswith("/query"):
                    return self.execute_query(token, body or {})
                else:
                    return self.create_database(token, body or {})
            elif method == "PUT":
                # Update database
                token = self.auth.authenticate_and_authorize(headers, APIPermission.WRITE)
                return self.update_database(token, body or {})
            elif method == "DELETE":
                # Delete database
                token = self.auth.authenticate_and_authorize(headers, APIPermission.DELETE)
                return self.delete_database(token, params)
            else:
                raise APIError(
                    message=f"Method {method} not allowed",
                    code=APIErrorCode.METHOD_NOT_ALLOWED,
