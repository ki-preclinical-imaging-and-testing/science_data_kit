"""
ISA Browser Page Module for Science Data Kit

This module provides the ISA Browser page for the Science Data Kit application.
The ISA Browser page handles browsing and managing ISA (Investigation, Study, Assay) 
ontology terms and their relationships with Neo4j.
"""
...
from science_data_kit.core.db.db_manager import db_manager, load_db_config
...
try:
    ...
    isatab, OntologyAnnotation, OntologySource, Investigation, Study, Assay, Process, Material, DataFile = get_isa_objects()
...
class IsaBrowserPage(BasePage):
    """
    ISA Browser page for browsing and managing ISA ontology terms.

    This page provides functionality for:
    - Loading and processing ISA-Tab files
    - Managing ontology terms from various sources (ISA, cBioPortal, OncoTree)
    - Extracting node classes, relationships, and properties from ISA objects
    - Integrating with Neo4j for storing and retrieving ontology terms
    - Exporting terms in various formats
    """

    def __init__(self):
        """Initialize the ISA Browser page."""
        super().__init__("ISA Browser", "🧪")
        self._setup_sidebar()
    ...
    def _setup_sidebar(self):
        """Set up the sidebar items for the ISA Browser page."""
    ...
    def _on_database_connect(self, uri: str, username: str, password: str, database: str):
        ...
        try:
            ...
            self.neo4j_connection = Neo4jConnection(config=neo4j_config)
    ...
    def render_content(self) -> None:
        """Render the ISA Browser page content."""
        ...
        with col1:
            ...
            with st.form("neo4j_connection_form"):
                ...
                if connect_button:
                    self._on_database_connect(uri, user, password, database)

        with col2:
            # Display connection status
            if st.session_state.neo4j_connected:
                st.success("✅ Connected to Neo4j")
                st.info(f"URI: {self.neo4j_connection.uri}")
                st.info(f"Database: {self.neo4j_connection.database}")
            ...
            if st.session_state.neo4j_connected:
                ...
                if st.button("Pull Terms from Neo4j", key="pull_terms_from_neo4j_button"):
                    try:
                        ...
                        LIMIT 100
                        """
                        term_results = self.neo4j_connection.execute_query(term_query)
                ...
                if st.button("Load Ontology Terms to Neo4j", key="load_ontology_terms_to_neo4j_button"):
                    if not st.session_state.terms:
                        ...
                        with st.spinner(f"Loading {len(st.session_state.terms)} ontology terms into Neo4j..."):
                            try:
                                relationships_created = self.neo4j_connection.load_ontology_relationships(
                                    st.session_state.terms,
...
RETURN s, r, t LIMIT 25
        ...
        if uploaded:
            try:
                ...
                # Nodes
                node_classes = self._extract_node_classes(inv)
                ...
                props = self._extract_properties(inv, selected_node)
                if props:
                ...
                rels = self._extract_relationships(inv)
                for rel in rels:
                    st.write(f"{rel[0]} -[{rel[1]}]-> {rel[2]}")
            except Exception as e:
                st.error(f"Error processing ISA file: {e}")
        else:
            st.info("Please upload an ISA-Tab or ISA-JSON file.")

def render_isa_browser_page():
