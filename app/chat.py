...
def chat():
    ...
    # LLM Settings Form
    with st.expander("LLM Connection Settings", expanded=False):
        with st.form("llm_settings_form"):
            ...
            with col2:
                ...
                llm_model = st.selectbox(
                    ...
                    index=0 if st.session_state["llm_model"] not in model_options else model_options.index(st.session_state["llm_model"])
                )
    ...
    if st.session_state["graph_rag"] is None and st.session_state["neo4j_uri"] and st.session_state["neo4j_user"] and st.session_state["neo4j_password"]:
    ...
    if st.session_state["graph_rag"] is None:
        ...
        # Display retriever type and schema information
        ...
        if "neo4j_schema" in st.session_state and st.session_state["neo4j_schema"]:
            with st.expander("Database Schema (used for Text2Cypher)", expanded=False):
                ...
                # if hasattr(st.session_state, "cached_nodes") and schema.node_labels:
                #     st.write(", ".join(schema.node_labels))
                # else:
                #     st.write("No node labels found in schema")
                #
                # # Display relationship types
                # st.subheader("Relationship Types")
                # if hasattr(schema, "relationship_types") and schema.relationship_types:
                #     st.write(", ".join(schema.relationship_types))
                # else:
                #     st.write("No relationship types found in schema")
                #
                # # Display node properties
                # st.subheader("Node Properties")
                # if hasattr(schema, "node_properties") and schema.node_properties:
                #     for label, properties in schema.node_properties.items():
                #         st.write(f"**{label}**: {', '.join(properties)}")
                # else:
                #     st.write("No node properties found in schema")
