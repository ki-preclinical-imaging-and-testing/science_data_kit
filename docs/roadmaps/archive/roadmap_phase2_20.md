# Science Data Kit (SDK) Phase 2 Roadmap - Version 20
...
| Task | Priority | Status | Notes |
|------|----------|--------|-------|
| Implement NExtSEEK integration | High | In Progress | Started work on connecting to NExtSEEK platform |
| Add FAIRDOM-Hub integration | High | In Progress | Started work on connecting to FAIRDOM-Hub platform |
...
| Task | Priority | Status | Notes |
|------|----------|--------|-------|
| Integrate with Jupyter notebooks | High | In Progress | Started work on improving the Jupyter notebook experience |
...
The complex property types implementation provides a flexible and powerful way to define, store, and validate complex property types in entities. The key features include:

1. **Property Type System**: A comprehensive type system with support for primitive types (string, integer, float, boolean, datetime), collection types (array, object), and specialized types (reference, geo_point, geo_shape, binary)
2. **Schema Validation**: Robust validation for all property types, including type checking, range validation, pattern matching, and nested validation
3. **Nested Structures**: Support for deeply nested object structures with their own schemas
4. **Array Support**: Support for array properties with item schemas for validating array elements
5. **Serialization/Deserialization**: Methods for converting complex properties to/from dictionaries and JSON
6. **Integration with BaseEntity**: Seamless integration with the existing entity system through the BaseEntity class
7. **Example Schema**: A comprehensive example schema for a person that demonstrates various complex property types and validation features

The data validation rules engine provides a comprehensive framework for defining and applying validation rules to data. The key features include:

1. **Validation Severity Levels**: Support for different severity levels (info, warning, error, critical) to distinguish between different types of validation issues
2. **Validation Results**: A structured way to represent validation results with information about validity, messages, severity, and paths
3. **Rule Types**: A variety of built-in rule types for common validation scenarios:
   - Required: Ensures a value is present and not None
   - Type: Validates the type of a value
   - Range: Validates numeric values are within a range
   - Length: Validates the length of strings or collections
   - Pattern: Validates strings against regular expression patterns
   - Enum: Validates values are from a set of allowed values
   - Custom: Allows custom validation logic
4. **Rule Sets**: A way to group related rules and apply them together
5. **Validation Context**: A high-level construct for validating complex data structures with multiple rule sets
6. **Path-based Validation**: Support for validating nested properties using dot notation paths
7. **Comprehensive Example**: An example of creating and using validation rules for a person

The user experience improvements provide a more intuitive and informative interface for the application. The key features include:

1. **Dashboard Overview**: A comprehensive dashboard that serves as the main entry point for the application, providing an overview of the application's features and status
2. **Service Status Visualization**: A visual representation of the connection status of various services, making it easy to see which services are connected and which are not
3. **Quick Access Cards**: Cards for common tasks that provide quick access to the most frequently used features, reducing the number of clicks needed to access key functionality
4. **Categorized Feature Organization**: Features organized into categories with tabs, making it easier for users to find and understand the available functionality
5. **Feature Tables**: Tables for each feature category that show the available features, their status, and descriptions, providing a clear overview of what the application can do
6. **Project Progress Visualization**: A visual representation of the progress of the project by category, helping users understand the current state of development
7. **Recent Activity Tracking**: A section that shows recent activities and updates to the project, keeping users informed about the latest changes
8. **Integrated Navigation**: The dashboard is integrated with the application's navigation, making it the default landing page and providing a consistent entry point for users
...
1. **Continue Integration Work**:
   - Complete NExtSEEK and FAIRDOM-Hub integrations
   - Finish Jupyter notebook integration
   - Implement additional platform integrations

2. **Begin Documentation Work**:
   - Create comprehensive user guide
   - Develop interactive tutorials
   - Update API documentation

3. **Performance Optimization** has been pushed back to focus on the above tasks first.
