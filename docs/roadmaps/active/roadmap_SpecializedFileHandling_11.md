# Science Data Kit (SDK) Specialized File Handling Roadmap - Version 11
...
2. **Enhance UI Integration**
   ...
   - Create faceted search for filtering search results by facets
...
The Specialized File Handling implementation has made significant progress, with the completion of the foundation phase, most of the core interpreters phase, and several key components of the UI integration phase. The implementation of the GeoTIFF interpreter, scientific data visualization component, metadata-based search and filtering, knowledge graph integration, and saved searches functionality has further enhanced the file handling capabilities of the Science Data Kit, particularly for scientific data files. The knowledge graph integration enables researchers to automatically enhance file nodes with specialized metadata, improving data discovery and relationship visualization. The saved searches functionality allows researchers to save and reuse complex search queries, making it easier to find relevant files. The next steps will focus on completing the remaining core interpreters, enhancing the UI integration, and beginning work on advanced features such as content extraction, similarity analysis, and automated tagging.