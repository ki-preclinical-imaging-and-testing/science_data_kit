# Science Data Kit (SDK) Microsoft Graph Extension Roadmap - Version 02
...
Most recently, the SharePoint document library integration has been completed with the implementation of SharePoint-specific methods in the MSGraphConnector class and a new SharePointFileManager class. This class provides a user-friendly interface for SharePoint document library operations, including listing files, getting metadata, downloading and uploading files, creating folders, deleting files/folders, and searching.

The next steps include enhancing the OneDrive file management capabilities and implementing Teams conversation analysis.
...
1. Complete the Core Integration with full support for entity schemas and mapping.
2. Enhance the Neo4j Knowledge Graph Integration for SharePoint and OneDrive data.
...
The Microsoft Graph extension has made significant progress in Phase 1 (Foundation) with the implementation of the core connector, file manager classes, and SharePoint integration. These components provide a solid foundation for the extension, enabling authentication, connection management, and file operations for both OneDrive and SharePoint document libraries. The next phases will build upon this foundation to provide a comprehensive integration with Microsoft 365 services, enhancing the Science Data Kit's capabilities for working with research data stored in Microsoft's cloud ecosystem.