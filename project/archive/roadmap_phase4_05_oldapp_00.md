# Science Data Kit (SDK) Phase 4 Roadmap - App Migration Sidequest
...
|------|----------|--------|-------|
| Migrate remaining functionality from `app.py` to `science_data_kit/ui/app.py` | High | To Do | Ensure all features and configurations are transferred |
| Migrate remaining functionality from `chat.py` to `science_data_kit/ui/pages/chat.py` | High | To Do | Verify all chat features work correctly after migration |
| Migrate remaining functionality from `explore.py` to `science_data_kit/ui/pages/explore.py` | High | To Do | Ensure data exploration features are fully functional |
...
| Migrate `menu.py` functionality to appropriate components | Low | To Do | Consider integrating with sidebar or navigation components |
...
| Task | Priority | Status | Notes |
|------|----------|--------|-------|
| Migrate remaining functionality from `app/utils/database.py` to `science_data_kit/core/db/` | High | To Do | Identify appropriate target files in the db directory |
| Migrate remaining functionality from `app/utils/db_adapter.py` to `science_data_kit/core/db/` | High | To Do | Consider merging with existing adapter classes |
| Verify `app/utils/db_manager.py` functionality is fully covered in `science_data_kit/core/db/db_manager.py` | High | To Do | Ensure all features are implemented in the new location |
...
| Task | Priority | Status | Notes |
|------|----------|--------|-------|
| Migrate `app/utils/models.py` to appropriate location in `science_data_kit/core/models/` | High | To Do | Ensure compatibility with existing model classes |
...
| Migrate `app/utils/sidebar.py` to `science_data_kit/ui/components/sidebar.py` | High | To Do | Ensure all sidebar functionality is transferred |
...
## Conclusion
This migration roadmap provides a structured approach to completing the transition from the legacy `app/` directory to the new `science_data_kit/ui/` structure. By following this plan, the project will achieve a more maintainable and organized codebase, aligning with the overall goals of Phase 4 of the Science Data Kit roadmap.