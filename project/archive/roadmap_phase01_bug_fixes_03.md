# Science Data Kit (SDK) Bug Fixes Roadmap - Phase 1 Version 03
...
5. Improve error handling for database procedures that may not be available in all Neo4j versions
6. Update deprecated Streamlit API calls
...
| Handle missing Neo4j procedures gracefully | High | Completed | Added specific error handling for the dbms.database.size() procedure when it's not available |

### 5. Streamlit API Updates

| Task | Priority | Status | Notes |
|------|----------|--------|-------|
| Update deprecated st.experimental_rerun() calls | High | Completed | Replaced with st.rerun() in sidebar.py to fix connection deletion error |
| Check for other deprecated Streamlit API calls | Medium | To Do | Need to review codebase for other deprecated API calls |
...
4. **Phase 4: Additional Bug Fixes**
   - Handle missing Neo4j procedures gracefully ✓
   - Update deprecated Streamlit API calls ✓
   - Update roadmap with new fixes ✓
...
6. The application now gracefully handles the case when the dbms.database.size() procedure is not available in the Neo4j instance
7. Deprecated `st.experimental_rerun()` calls have been replaced with `st.rerun()` to fix connection deletion error

## Next Steps

1. Continue monitoring for any additional issues that may arise
2. Check for other deprecated Streamlit API calls in the codebase
3. Consider implementing proper container management features in a future update
4. Investigate resource usage to prevent application termination (SIGKILL)
5. Develop comprehensive testing procedures to ensure all fixes remain stable
6. Update tests to reflect the current state of the application
...
6. Neo4j procedure calls are handled gracefully when procedures are not available
7. All Streamlit API calls use current, non-deprecated methods